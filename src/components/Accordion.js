...
const Accordion = ({ links, updateBackground, updateTheme, updateParallax, updateAudio, updateSplash, updateHeader, settings, updateFight, updateDrawerBtn, unlockSecret }) => {
  const handleClick = (item) => {
    switch(item.id) {
      ...
      case 'modal': unlockSecret({ modal: item.value }); break;
  ...
  const handleActive = (item) => {
    const { audio, background, fight, header, parallaxes, showSplash, theme, drawerBtn, secretUnlock } = settings;
    switch(item.value) {
      ...
      case secretUnlock.modal: return 'active';
