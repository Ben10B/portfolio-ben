...
const Accordion = ({ links, updateBackground, updateTheme, updateParallax, updateAudio, updateSplash, updateHeader, settings, updateFight, updateDrawerBtn, unlockSecret }) => {
  const handleClick = (item) => {
    switch(item.id) {
      ...
      case 'controls': updateAudio({ controls: item.value }); break;
  ...
  return (
    <div className={`accordion`}>
      <div className="menu">
        <ul>
          {links.map(x => (
            <li key={x.id} className="item" id={x.id} onClick={x.lvl2 ? null : () => handleClick(x)}>
              <div className="btn">
              ...
              {x.lvl2 && (
                <div className="smenu">
                  ...
                  : 
                  (x.lvl2.map(y => (
                    <div key={y.value} className="item-2" id={y.id} onClick={() => handleClick(y)}>
